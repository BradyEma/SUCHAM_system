                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($wishlistItems as $item)
                            <div class="wishlist-item bg-white border border-gray-200 rounded-lg overflow-hidden transition duration-300">
                                ...
                                <div class="p-4">
                                    <h3 class="font-semibold text-lg text-gray-800 mb-1">{{ $item->product->product_name }}</h3>
                                    <div class="flex justify-between items-center mb-2">
                                        <span class="text-primary-600 font-bold">UGX {{ number_format($item->product->price) }}</span>
                                        <span class="stock-badge {{ $item->product->quantity > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
